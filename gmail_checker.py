...
class InvoiceDB:
    ...
    def init_db(self):
        ...
        try:
            cursor.execute("ALTER TABLE invoices ADD COLUMN status TEXT DEFAULT 'PENDING_REVIEW'")
        ...
        try:
            cursor.execute('''
                INSERT INTO invoices (
                    ...
                    line_items_json, found_in_spam, email_message_id, status
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
...
def send_invoice_alert(invoice_data, logger):
    """
    Send email alert to owner about new invoice.
    """
    try:
        ...
        <p>Please review this invoice in your dashboard.</p>
        </body></html>
        """
        
        message = MIMEText(body, 'html')
        message['to'] = settings.OWNER_EMAIL
        message['subject'] = f"New Invoice: {vendor} (${amount})"
        raw = base64.urlsafe_b64encode(message.as_bytes()).decode('utf-8')
        
        service.users().messages().send(userId='me', body={'raw': raw}).execute()
        logger.info(f"Sent alert to owner for invoice {invoice_data.get('invoice_number')}")
        ...
        for email_data, is_spam in all_emails:
            ...
            try:
                ...
                if success:
                    ...
                    # Send Alert
                    send_invoice_alert(invoice_data, logger)
