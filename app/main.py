...
async def lifespan(app):
    ...
    # 0. Auto-seed admin user — default: admin@procureiq.com / admin123
    #    Override with ADMIN_EMAIL + ADMIN_PASSWORD env vars in Railway.
    try:
        import hashlib as _hl
        admin_email    = os.environ.get("ADMIN_EMAIL", "admin@procureiq.com")
        ...
        try:
            ...
            if not existing:
                ...
                db.commit()
                print(f"[STARTUP] Admin password set for: {admin_email}")
            else:
                print(f"[STARTUP] Admin already exists: {admin_email}")
        finally:
            db.close()
